...
def apriori(dataset, min_support, verbose=False):
    ...
    print 'Candidates C1 : \n', C1
    D = map(set, dataset)
    F1a, a_support_data = support_prune(D, C1, minsup, verbose=False) # prune candidate 1-itemsets
    print 'Pruned Candidates are F1: \n', F1a, a_support_data
    Fa = [F1a] # list of frequent itemsets; initialized to frequent 1-itemsets
    print 'Pruned Candidates are Fa: \n', Fa
    k = 2 # the itemset cardinality
    while (len(Fa[k - 2]) > 0):
        Ck = apriori_gen(Fa[k-2], k) # generate candidate itemsets
        print 'Generated candidate itemsets are Ck: \n',Ck
        Fk, supK = support_prune(D, Ck, minsup) # prune candidate itemsets
        print 'Pruned candidate itemsets are Fk: \n',Fk, supK
        a_support_data.update(supK) # update the support counts to reflect pruning
        Fa.append(Fk) # add the pruned candidate itemsets to the list of frequent itemsets
        print 'Pruned itemsets are: \n', Fa
        k += 1

    ##print 'Final Pruned itemsets: Support data \n ', F , ':', support_data
        ...
        for kset in Fa:
            for item in kset:
                print("" \
                    ...
                    + "}" \
                    + ":  sup = " + str(round(a_support_data[item], 3)))

    print 'Final Pruned itemsets: Support data \n ', Fa , ':', a_support_data
    return Fa, a_support_data
...
def create_candidates(dataset, verbose=False):
    ...
    if verbose:
        ...
        print 'Printing a list of all the candidate items from create_candidates()'
        print("" \
            + "{" \
            + "".join(str(i[0]) + ", " for i in iter(c1)).rstrip(', ') \
            + "}")
        print 'End of candidate items \n'

    # Map c1 to a frozenset because it will be the key of a dictionary.
    return map(frozenset, c1)

def support_prune(dataset, candidates, minsup, verbose=False):
    ...
    if verbose:
        ...
        print 'End of Pruned Candidates\n'
...
def rules_from_conseq(freq_set, H, support_data, rules, min_confidence, verbose=False):
...
def readCSV(filename):
    '''Reading csv file and converting them into a list of lists like the above list.
...
def main():
    ...
    dataset, count = readCSV(filename)

    print 'Total Transactions = ', count



    print 'Dataset used is :\n',dataset
    ...
    C1 = create_candidates(D, verbose=True) # candidate 1-itemsets
    F1, support_data = support_prune(D, C1, minsup, verbose=True)
    F2, support_data2 = apriori(D, minsup, verbose=True)
    print 'F2 =  '
    print ' '
    print ' '
    print F2
    ...
    print ' '
    print support_data2
    H = generate_rules(F2, support_data2, min_confidence, verbose=True)
